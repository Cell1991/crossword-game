...
export const TurnBanner: React.FC<TurnBannerProps> = ({
  ...
  return (
    <div className="flex items-center gap-2">
      {isMyTurn ? (
        <div className="flex items-center gap-2 whitespace-nowrap rounded-full border border-emerald-400/50 bg-emerald-500/20 px-3 py-1 text-xs font-semibold text-emerald-200 shadow-md shadow-emerald-950/30 animate-pulse sm:text-sm">
          <Sparkles className="w-4 h-4 text-emerald-400" />
          <span>YOUR TURN</span>
          <span className="font-mono text-xs text-emerald-300/80">TURN {turnNumber}{maxTurns ? ` / ${maxTurns}` : ''}</span>
        </div>
      ) : (
